      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">Data Pequrban & Ramadhan</h3>
          </div>
          <!-- /.box-header -->
          <div class="table-responsive">
            <table id="example1" class="table table-hover">
             ...
             <td>{{Str::limit($donatur->judul,20)}}</td>
             <td>{{$donatur->name}}</td>
             <td>{{$donatur->nama_qurban}}</td>
             <td>{{$donatur->email}}</td>
             <td>{{$donatur->nohp}}</td>
             <td hidden>{{$donatur->kode}}</td>
             <td>{{Str::limit($donatur->bank,10)}}</td>
             <td hidden>{{$donatur->id_qurban}}</td>
             ...
             <td>
              ...
              <div class="btn-group">
                @if ($donatur->status == 1)
                <a class="btn btn-danger" href="{{ url('/statusrm/'.$donatur->id_pequrban) }}">Pending</a>
                @else
                <a class="btn btn-primary" href="{{ url('/statusrm/'.$donatur->id_pequrban) }}">Acc</a>
                @endif
              
              </div>
            <form method="post" action="{{url('/dataramadhan/'.$donatur->id_pequrban)}}">
                @csrf
                @method('DELETE')
